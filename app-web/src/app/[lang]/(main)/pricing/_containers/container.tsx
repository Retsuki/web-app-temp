...
export const PricingContainer = ({ dict }: { dict: Dictionary }) => {
	...
	return (
		<div className="space-y-8">
			...
			<div className="grid md:grid-cols-3 gap-8">
				...
				{/* Starter Plan */}
				<Card
					className={subscription?.plan === "starter" ? "border-primary" : ""}
				>
					<CardHeader>
						<CardTitle>{starterPlan?.name}</CardTitle>
					...
					<CardContent className="space-y-4">
						<div>
							...
							<span className="text-muted-foreground">
								/{dict.pricing.period[billingCycle]}
							</span>
						</div>
						<ul className="space-y-2">
							<li className="flex items-start gap-2">
							...
							<li className="flex items-start gap-2">
								<CheckIcon className="h-5 w-5 text-green-500 flex-shrink-0 mt-0.5" />
								<span className="text-sm">
									{dict.pricing.features?.storage || "Storage"}
								</span>
							</li>
						</ul>
					</CardContent>
					<CardFooter>
						<Button
							className="w-full"
							variant={subscription?.plan === "starter" ? "outline" : "default"}
				...
				{/* Pro Plan */}
				<Card
					className={`relative ${subscription?.plan === "pro" ? "border-primary" : ""}`}
				>
