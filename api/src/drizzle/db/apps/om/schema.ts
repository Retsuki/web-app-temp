...
export const omCreditBalances = pgTable(
	"om_credit_balances",
	{
		...
		totalAllocated: decimal("total_allocated", { precision: 20, scale: 2 })
			.notNull()
			.default("0"),
		totalConsumed: decimal("total_consumed", { precision: 20, scale: 2 })
			.notNull()
			.default("0"),
	...
	(table) => {
		return {
			endUserIdIdx: index("om_credit_balances_end_user_id_idx").on(
				table.endUserId,
			),
		};
	},
...
export const omCreditTransactions = pgTable(
	...
	(table) => {
		return {
			endUserIdIdx: index("om_credit_transactions_end_user_id_idx").on(
				table.endUserId,
			),
			typeIdx: index("om_credit_transactions_type_idx").on(table.type),
			createdAtIdx: index("om_credit_transactions_created_at_idx").on(
				table.createdAt,
			),
		};
	},
...
export const omMeterEvents = pgTable(
	...
	(table) => {
		return {
			...
			endUserIdIdx: index("om_meter_events_end_user_id_idx").on(
				table.endUserId,
			),
...
export const omSubscriptions = pgTable(
	...
	(table) => ({
		...
		stripeSubscriptionIdIdx: index(
			"om_subscriptions_stripe_subscription_id_idx",
...
export const omPaymentHistory = pgTable(
	...
	(table) => ({
		...
		stripePaymentIntentIdIdx: index(
			"om_payment_history_stripe_payment_intent_id_idx",
		).on(table.stripePaymentIntentId),
		createdAtIdx: index("om_payment_history_created_at_idx").on(
			table.createdAt,
		),
	}),
);
...
export const omWebhookEvents = pgTable(
	...
	(table) => ({
		stripeWebhookEventIdIdx: index(
			"om_webhook_events_stripe_webhook_event_id_idx",
		).on(table.stripeWebhookEventId),
...
export const omPlanLimits = pgTable(
	...
	(table) => ({
		planFeatureUnique: unique("om_plan_limits_plan_feature_unique").on(
			table.planId,
			table.feature,
		),
	}),
...
// クレジット残高のリレーション
export const omCreditBalancesRelations = relations(
	omCreditBalances,
	({ one }) => ({
...
// クレジット取引のリレーション
export const omCreditTransactionsRelations = relations(
...
// 使用量集計のリレーション
export const omUsageAggregationsRelations = relations(
	omUsageAggregations,
	({ one }) => ({
		project: one(omProjects, {
...
// サブスクリプションのリレーション
export const omSubscriptionsRelations = relations(
	omSubscriptions,
	({ one }) => ({
		user: one(profiles, {
...
// 支払い履歴のリレーション
export const omPaymentHistoryRelations = relations(
	omPaymentHistory,
	({ one }) => ({
		user: one(profiles, {
