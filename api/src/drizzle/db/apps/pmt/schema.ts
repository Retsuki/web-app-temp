...
export const pmtPosts = pgTable(
  'pmt_posts',
  {
    ...
    selectedSnsAccountIds: jsonb('selected_sns_account_ids').default([]).$type<string[]>(),
...
export const pmtAiAgents = pgTable(
  'pmt_ai_agents',
  {
    ...
    avatarBgColor: varchar('avatar_bg_color', { length: 50 }).default('#FFFFFF'),
  ...
  (table) => ({
    ...
    durationCheck: check('duration_check', sql`${table.duration} >= 1 AND ${table.duration} <= 60`),
  })
)
...
export const pmtAiAgentSnsAccounts = pgTable(
  ...
  (table) => ({
    ...
    accountIdx: index('pmt_ai_agent_sns_accounts_account_idx').on(table.snsAccountId),
    platformIdx: index('pmt_ai_agent_sns_accounts_platform_idx').on(table.platform),
...
export const pmtAiAgentContentGenerationSchedules = pgTable(
  'pmt_ai_agent_content_generation_schedules',
  {
    ...
    timeZone: varchar('time_zone', { length: 100 }).notNull().default('Asia/Tokyo'),
...
export const pmtPaymentHistory = pgTable(
  'pmt_payment_history',
  {
    ...
    // サブスクリプション関連
    subscriptionId: uuid('subscription_id').references(() => pmtSubscriptions.id, {
      onDelete: 'set null',
    }),
...
export const pmtOauthStates = pgTable(
  ...
  (table) => {
    return {
      ...
      userProjectIdx: index('idx_oauth_states_user_project').on(table.userId, table.projectId),
...
export const pmtPostTargets = pgTable(
  ...
  (table) => {
    return {
      // 同一投稿・同一アカウントの重複を防ぐ
      uniquePostAccount: unique('unique_post_account').on(table.postId, table.snsAccountId),
      postIdIdx: index('idx_post_targets_post_id').on(table.postId),
      snsAccountIdIdx: index('idx_post_targets_sns_account_id').on(table.snsAccountId),
      statusIdx: index('idx_post_targets_status').on(table.status),
      scheduledAtIdx: index('idx_post_targets_scheduled_at').on(table.scheduledAt),
...
export const pmtAiAgentPostSchedulesRelations = relations(pmtAiAgentPostSchedules, ({ one }) => ({
  agent: one(pmtAiAgents, {
...
export const pmtSubscriptionsRelations = relations(pmtSubscriptions, ({ one, many }) => ({
...
export const pmtPaymentHistoryRelations = relations(pmtPaymentHistory, ({ one }) => ({
  profile: one(profiles, {
...
export const pmtAiAgentSnsAccountsRelations = relations(pmtAiAgentSnsAccounts, ({ one }) => ({
  agent: one(pmtAiAgents, {
